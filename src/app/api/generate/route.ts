...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await client.chat.completions.create({
      model: "deepseek-chat",
      messages: [
        { role: "system", content: `You are an expert Content Marketing Strategist. Generate a comprehensive content calendar from the user's inputs.\n\nFor each piece of content, specify:\n- Content type (blog post, social post, video, email, etc.)\n- Specific topic/title\n- Platform\n- Optimal posting day/time\n- Key message or CTA\n- Format notes\n\nOrganize by week. Also include content theme per week, campaign highlights, variety of formats, and engagement tips. Make it actionable and specific.` },
